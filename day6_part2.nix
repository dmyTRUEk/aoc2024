let
	...
	loop_step = m:
		if (get_guard_yx m) == null then
			...
			loop_step (step m)
	;
	# is_looped_ = m_initial: m:
	# 	if (get_guard_yx m) == null then
	# 		false
	# 	else if (get_guard_state m) == (get_guard_state m_initial) then
	# 		true
	# 	else
	# 		is_looped_ m_initial (step m)
	# ;
	# is_looped = m: is_looped_ m (step m);
	is_looped_ = states: m:
	...
	trace_indices = m_traced:
		indices_of_in_arr2d (eq "X") m_traced
	;
in
	input:
	let
		...
		m_trace_indices = m_initial |> loop_step |> trace_indices |> filter (ne guard_pos);
	in
		m_trace_indices
